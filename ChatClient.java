...
public class ChatClient implements Runnable, ChatClientInterface {
    ...
    public void run() {
		try {
			        ...
			        Socket socket = new Socket(getServerAddress(), getServerPort());  // Create the socket

			        // TODO STUDENT: setup input and output streams
			        in = new BufferedReader(new InputStreamReader(socket.getInputStream()));  // Set up the input stream
			        out = new PrintWriter(socket.getOutputStream(), true);  // Set up the output stream, auto-flush enabled
